...
Dams are an integral part of the modern economy as they satisfy the water demands of a growing population. In addition to providing hydroelectricity and recreational services, they are crucial for ensuring enough water supply in areas where water is scare and, for preventing floods in areas with abundant rainfall and storms. Dams are central to storing and managing US waters. Currently, US has a network of 90,000 dams spread across its 50 states, about two-thirds of which were constructed more than 50 years ago. Aging dams can be more prone to failure and can pose risk to downstream cities. Moreover, at the time of their construction, they might have been in rural areas where a failure would not have caused any damage to life and property. However, since then there has been a growth in urbanization which increase the vulnerability of the population living near the dams. 

Research Question I: Which physical characteristics of a dam influence its hazard potential? 

Realizing the socio-economic risks of a dam failure, the Congress passed the National Dam Inspection Act in 1972 which authorizes the US Army Corps of Engineers (USACE) to create dam inventory of the US. Since then various acts have passed to improve funding for improving data collection and interpretation efforts. The 2019 National Inventory of Dam (NID) dataset includes an inventory of 91,457 dams and classifies them as low, significant, and high-risk potential. A dam is a high safety hazard if its failure is likely to bring a significant damage to human property and loss of life in downstream areas. In addition to criteria determined by USACE, this analysis investigates several features of a dam and explore which dam characteristics can predict can its hazard potential.

Research Question II: What is the spatial distribution of North Carolina dams with varying hazard potential in hydrologic units with temporal streamflow trends?

One of the common reasons of a dam failure is overtopping due to excessive storms or floods exceeding the capacity of dams. Such gravitation movement of water over the dams can damage the foundation of the dam thereby making it more susceptible to failure. On the other hand, low water availability risks all the services that a dam provides. This paper investigated the temporal trends of water flows in North Carolina sub-basins and determined how the dams with varying hazard potential are distributed across sub-basins of varying water flow trends.   
...
The second dataset included water run-off data for US sub basins and was downloaded from the WaterWatch platform hosted by United States Geological Survey (USGS) website at https://waterwatch.usgs.gov/index.php?id=romap3&sid=w__download. The dataset was downloaded in a tab separated text file called “wy01d_col_data.txt”. It contained 2,111 columns for every HUC-8 unit in the US and the first column being the years (1901 – 2019) for which water run-off was calculated. The downloaded folder also contained a PDF file which included detailed instructions of how water run-off for every sub-basin was calculated. 
...
ggplot(log_dam_characteristics) +
  geom_boxplot(aes(x=hazard, y = volume),
               fill = c("coral","cyan","darkgoldenrod1")) +
  labs (x = "Hazard", y = "Log (Volume (cubic yards))") +
  peaceful.theme
...
ggplot(log_dam_characteristics) +
  geom_boxplot(aes(x=hazard, y = drainage_area),
               fill = c("coral","cyan","darkgoldenrod1")) +
  labs (x = "Hazard", y = "Log (Drainage area (sq-miles))") +
  peaceful.theme
...
The dataset containing water run-off time series for all US sub-basins was imported. Its columns was renamed by erasing the letter 'X' in front of HUC-8 number. A shapefile for NC sub-basins was used to extract run-off data for North Carolina HUC-8 units only. The newly obtained NC gage data was stored in a separate dataframe. It was converted into a time series object for monotonic trend analysis.

```{r}
# Importing gage time series data
huc8 <- read.csv("Mishra_FinalProject_ENV.872/Data/Raw/wy01d_col_data.txt", 
                 sep = "\t")
...
From the 2019 NID dam dataset used above, North Carolina dams were subsetted along with their relevant geographical information. Using the 'sf' package it was converted from tabular data to sf object. Then using the US state boundary shapefile, NC boundary was stored in a new sf object and relevant coordinate reference systems were assigned.
...
state_bound_raw <- st_read("Mishra_FinalProject_ENV.872/Data/Raw/state_bounds.shp")
...
Since the safety hazard of dams follows an order of severity, which is High (H) > Significant (S) > Low (L), and because it is a factor variable, it was determined that an ordinal logistic regression (OLR) would be the appropriate model to investigate the effects of physical dam features on hazard potential. The predictor variables chosen were a dam's volume, storage capacity, spillway width, drainage area, architectural height, surface area and dam length. 5 different OLR models were tested by selecting a combination of aforementioned variables. However, all-inclusive OLR model returned the lowest AIC value. There, all the variables were included. The 'polr' function used to do OLR did not return p-values. Therefore, p-values were separately by using t-value against standard normal distribution.
...
From the p-values of the model results, we can observe that except the spillway width and surface area of a dam, all other variables have a significant effect on the safety hazard of a dam. The OLR model coefficients are exponentiated below for easy interpretation. For instance, we can say that with one unit increase of volume, the log of odds of having a higher safety hazard of dams increases by a factor of 1.03. Similarly, one unit increase in storage, drainage area, height and length affects the log of odds of increasing dam safety risk by a factor of their respective exponentiated coefficients.

Predictors | Exponentiated OLR coefficient
-----------|------------------------------
Volume | 1.03
Storage | 0.82
Drainage area | 0.97
...
This study focused on understanding the physical determinants of dam safety hazards. In addition, it aimed to understand the temporal changes in run-off that can affect the dam operations. It was found that volume, storage, drainage area, height and length have a siginificant effect on hazard potential of dams which might be intuitive as failure of larger dams can cause greater damages to life and property. In addition it was found that a majority of dams irrespective of their safety hazard are located in sub-basins witnessing a decereasing trend in annual discharge. While decreasing water availability prevents a dam to operate on its full potential, they also decrease the chances of dam failures due to overtopping. Similar analyis can be performed for other states as well in order to focus on high risk dams situation in areas with increasing water flows and developing management strategies to prevent dam failures.
