...
The second dataset included water run-off data for US sub basins and was downloaded from the WaterWatch platform hosted by United States Geological Survey (USGS) website at https://waterwatch.usgs.gov/index.php?id=romap3&sid=w__download. The dataset was downloaded in a tab separated text file called “wy01d_col_data.txt”. It contained 2,111 columns for every HUC-8 unit in the US and the first column being the years (1901 – 2019) for which water run-off was calculated. The downloaded folder also contained a PDF file which included detailed instructions of how water run-off was calculated for every sub-basin. 
